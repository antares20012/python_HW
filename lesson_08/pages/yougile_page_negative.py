...
class YougileApi:
    ...
    def create_yougile_my_key(self):  # ПОЛУЧЕНИЕ ключей КОМПАНИИ NEGATIVE
    ...
    def create_project_negative(self, title):  # СОЗДАНИЕ проекта NEGATIVE, отсутстыует  токен авторизации
        payload = {
            "title": "QA Automation Project"
        }
        response = requests.post(self.url + '/projects', json=payload)
        self.last_status_code = response.status_code
        # response.raise_for_status()
    ...
    def create_project_positive(self, title):  # СОЗДАНИЕ проекта
        payload = {
            "title": "QA Automation Project"
        }
    ...
    def get_last_project_title_positive(self):
    ...
    def get_last_project_title_negative(self):
        if not self.last_project_id:
            return None
    ...
    def update_project_negative(self, new_title):
        if not self.last_project_id:
            return None
        url = f"{self.url}/projects/{self.last_project_id}"
        payload = {"title": new_title}
